...
local combatbox  = tabs.combat:AddLeftGroupbox("aimbot", "crosshair")
local combatbox2 = tabs.combat:AddRightGroupbox("misc",  "zap")

local espbox    = tabs.visuals:AddLeftGroupbox("esp",     "eye")
local visualbox = tabs.visuals:AddRightGroupbox("visuals","sparkles")
local aabox     = tabs.visuals:AddLeftGroupbox("antiaim", "shield")

local skinbox  = tabs.skins:AddLeftGroupbox("skins", "swords")
local knifebox = tabs.skins:AddRightGroupbox("knife","scissors")

local uibox     = tabs.settings:AddLeftGroupbox("ui settings", "settings")
local cfgbox    = tabs.settings:AddLeftGroupbox("configs",     "save")
...
combatbox:AddSlider('trig_d', { Text = 'trig delay',  Default = 0, Min = 0, Max = 500, Rounding = 0 }):OnChanged(function() trigger_delay = options.trig_d.Value end)
...
combatbox2:AddToggle('bhop', { Text = 'bhop', Default = false }):OnChanged(function() bhop_enabled = toggles.bhop.Value end)
runservice.RenderStepped:Connect(function()
...
combatbox2:AddToggle('nr', { Text = 'no recoil', Default = false }):OnChanged(function() norecoil_enabled = toggles.nr.Value end)
...
combatbox2:AddToggle('wb', { Text = 'wallbang', Default = false }):OnChanged(function()
    ...
    if wallbang_enabled then
        for _, v in ipairs(workspace_svc:GetDescendants()) do
            if v:IsA("BasePart") then
                ...
                for _, kw in ipairs(wallbang_keywords) do
                    if string.find(name, kw, 1, true) then v.CanCollide = false; v.CastShadow = false; break end
                end
            end
        end
    else
        for _, v in ipairs(workspace_svc:GetDescendants()) do
            if v:IsA("BasePart") then
                local name = string.lower(v.Name)
                for _, kw in ipairs(wallbang_keywords) do
                    if string.find(name, kw, 1, true) then v.CanCollide = true; break end
                end
            end
        end
    end
...
combatbox2:AddButton('wallbang', function()
    for i, v in ipairs(workspace_svc:GetDescendants()) do
        local name = string.lower(v.Name)
        if string.find(name, "cube") or string.find(name, "wall") or string.find(name, "box") or
           string.find(name, "crate") or string.find(name, "fence") or string.find(name, "container") or
           string.find(name, "concrete") or string.find(name, "cube.001") or string.find(name, "ship") or
           string.find(name, "invisible") or string.find(name, "plane.002") or string.find(name, "plane.003") or
           string.find(name, "ceiling.006") or string.find(name, "acprop") or string.find(name, "cylinder.008") or
           string.find(name, "doorarchway.001") or string.find(name, "door3_low") or string.find(name, "cylinder.006") then
            v:Destroy()
        end
    end
end)
...
runservice.RenderStepped:Connect(function()
    ...
    for _, v in ipairs(target_folder:GetChildren()) do
        ...
        if hum and hum.Health > 0 and root and hd then
            ...
            if vis then
                local bh  = math.abs(hp_at.Y - lp_at.Y)
                local bw  = bh / 2
                ...
                if show_hp then
                    ...
                    local bx  = rp.X - bw/2 - 6
                    esp.hp_out.From = Vector2.new(bx, hp_at.Y-1);    esp.hp_out.To = Vector2.new(bx, hp_at.Y + bh + 1); esp.hp_out.Visible = true
                    esp.hp_bar.From = Vector2.new(bx, hp_at.Y + bh); esp.hp_bar.To = Vector2.new(bx, hp_at.Y + bh - (bh*hpp))
                    esp.hp_bar.Color = Color3.new(1-hpp, hpp, 0);    esp.hp_bar.Visible = true
...
espbox:AddToggle('esp_h', { Text = 'hp',   Default = true  }):OnChanged(function() show_hp     = toggles.esp_h.Value end)
espbox:AddToggle('esp_n', { Text = 'name', Default = true  }):OnChanged(function() show_name   = toggles.esp_n.Value end)
espbox:AddToggle('esp_d', { Text = 'dist', Default = true  }):OnChanged(function() show_dist   = toggles.esp_d.Value end)
...
local function show_hitmarker()
    ...
    for i, l in ipairs(hm_lines) do
        l.From = c + defs[i][1]; l.To = c + defs[i][2]; l.Visible = true
    end
    task.delay(0.12, function() for _, l in ipairs(hm_lines) do l.Visible = false end end)
end
...
inputservice.InputBegan:Connect(function(i, gp)
    if gp then return end
    if i.UserInputType == Enum.UserInputType.MouseButton1 then task.spawn(do_raycast_and_fire) end
end)
...
visualbox:AddSlider('mb_amt', { Text = 'blur amount', Default = 15, Min = 1, Max = 45, Rounding = 0 }):OnChanged(function() blur_amount    = options.mb_amt.Value end)
visualbox:AddSlider('mb_amp', { Text = 'blur amp',    Default = 5,  Min = 1, Max = 15, Rounding = 0 }):OnChanged(function() blur_amplifier = options.mb_amp.Value end)
...
aabox:AddToggle('aa',        { Text = 'anti aim', Default = false }):OnChanged(function() antiaim_enabled    = toggles.aa.Value end)
aabox:AddDropdown('aa_mode', { Text = 'aa mode',  Values = {"Spin","Jitter","Down","Up"}, Default = "Spin" }):OnChanged(function() antiaim_mode       = options.aa_mode.Value end)
aabox:AddSlider('aa_spd',    { Text = 'spin spd', Default = 10, Min = 1, Max = 30, Rounding = 0 }):OnChanged(function() antiaim_spin_speed = options.aa_spd.Value end)
...
knifebox:AddDropdown('knf_sel', { Text = 'knives', Values = {"Butterfly Knife","Karambit","M9 Bayonet","Flip Knife","Gut Knife"}, Default = "Butterfly Knife" }):OnChanged(function()
...
thememanager:ApplyToGroupbox(uibox)
savemanager:ApplyToGroupbox(cfgbox)
